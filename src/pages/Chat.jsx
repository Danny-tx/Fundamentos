...
function Chat() {
    ...
    // Presencia del otro usuario en el chat
        ...
        const run = async () => {
            ...
            presenceChannel
                ...
                .on('presence', { event: 'join' }, ({ newPresences }) => {
                    if (newPresences.some(p => p.user_id !== user.id)) setOtherUserOnline(true)
                })
                            ...
                            return (
                                <div key={msg.id} className="msg-row msg-anim"
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px', paddingInline: '4px' }}>
                                        ...
                                        {isOwn && !isDeleted && (
                                            <span
                                                title={msg.read_count > 0 ? 'Visto' : otherUserOnline ? 'Entregado' : 'Enviado'}
                                                style={{
                                                    fontSize: '12px',
                                                    color: msg.read_count > 0 ? '#3b82f6' : '#71717a',
                                                    fontWeight: 600,
                                                    letterSpacing: '-1px'
                                                }}>
                                                {msg.read_count > 0 ? '✓✓' : otherUserOnline ? '✓✓' : '✓'}
                                            </span>
                                        )}
                                    </div>
                                </div>
